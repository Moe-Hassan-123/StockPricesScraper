...
def gui():
    ...
    Ticker_btn = tk.Button(
        Historic, text="Switch to Ticker Mode", command=change_to_Ticker, font=small_font
    )
    Ticker_btn.pack(pady=20, padx=15, anchor=tk.N)

    Historic_btn = tk.Button(
        Ticker, text="Switch to Historic Mode", command=change_to_historic, font=small_font
    )
    ...
    ticker_label.pack()
    tk.Button(Ticker, text="Refresh", font=(small_font), command=refresh).pack(
        pady=20, padx=15, side=tk.LEFT, anchor=tk.N
    )
    ...
    tk.Button(
        ...
        text="Submit",
        font=(small_font),
        command=lambda: get_ticker_data(my_entry.get().strip().upper()),
    ).pack(pady=20, padx=15, side=tk.LEFT, anchor=tk.N)
    ...
    historic_label.lift()
    tk.Button(Historic, text="All", font=small_font, command=get_all_data, padx=25).pack(
        pady=20, fill=tk.X
    )
...
def compile_data(today_data):
    ...
    for ticker in today_data.index:
        if ticker in historic_df.index:
            ...
            percentage = (highest_price_after_historic_low - float(lowest_historic_price)) / float(
